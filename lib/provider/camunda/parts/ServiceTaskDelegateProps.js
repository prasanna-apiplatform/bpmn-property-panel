                ...
                try {
                    ...
                    if (element['businessObject']['name'] !== undefined){
                        ...
                        if (element['businessObject']['name'] !== 'Select' ){
                            // console.log('2');
                            // console.log(element);
                            // localStorage.setItem(element['businessObject']['id'], '');
                            // console.log(localStorage.getItem(element['businessObject']['id']));
                            // console.log(element['businessObject']['name']);
                            if (localStorage.getItem(element['businessObject']['id']) !== element['businessObject']['name'])
                            {
                                localStorage.setItem(element['businessObject']['id'], element['businessObject']['name']);
                                // console.log('entered inside new creation');
                                // console.log(localStorage.getItem(element['businessObject']['id']));
                                ...
                                var sqlQueryAPI = localStorage.getItem('sqlQueryAPI');
                                sqlQueryAPI = JSON.parse(sqlQueryAPI);

                                var businessLogicAPI = localStorage.getItem('businessLogicAPI');
                                businessLogicAPI = JSON.parse(businessLogicAPI);

                                var b2bAPI = localStorage.getItem('b2bAPI');
                                b2bAPI = JSON.parse(b2bAPI);

                                // console.log(b2bAPI);
                                var totalAPIs = []
                                for ( let i = 0 ; i < dataAccessAPI.length ; i++){totalAPIs.push(dataAccessAPI[i]);}
                                for ( let i = 0 ; i < sqlQueryAPI.length ; i++){totalAPIs.push(sqlQueryAPI[i]);}
                                for ( let i = 0 ; i < businessLogicAPI.length ; i++){totalAPIs.push(businessLogicAPI[i]);}
                                for ( let i = 0 ; i < b2bAPI.length ; i++){
                                    for ( let j = 0 ; j < b2bAPI[i]['executors'].length ; j++){
                                        totalAPIs.push(b2bAPI[i]['executors'][j]);
                                    }
                                }
                                var obj = totalAPIs.find(o => o.executor_name === selected_exe_name && o.executor_version === selected_exe_version);
                                // console.log(obj);
                                if (obj === undefined){
                                    // console.log('entered into undefined')
                                    var selected_exe_name = APIname.split('_');
                                    selected_exe_name  =selected_exe_name[2];
                                    var selected_exe_names = APIname.split('_');
                                    selected_exe_version  =selected_exe_names[3];
                                    // console.log(selected_exe_version);
                                    // console.log(selected_exe_name);
                                    var obj = totalAPIs.find(o => o.executor_name === selected_exe_name && o.executor_version === selected_exe_version);
                                    // console.log(obj);
                                }
                                // console.log(totalAPIs);
                                // console.log(obj);
                                ...
                                for (let i = 0 ; i < obj['headers'].length ; i++){
                                    var newEntry2 = createElement('camunda:Entry', 'inputParameters', bpmnFactory, { key: obj['headers'][i]['key'], value: obj['headers'][i]['value'] });
                                    testt['entries'].push(newEntry2);
                                }
                                // console.log(testt);
